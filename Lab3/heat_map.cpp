...
void Pen_Color (QPainter* p,float sigPower)
{
    if(sigPower >= -54)
    {
        p->setPen(QColor(255, 0, 0, 255)); // красный
    }
    ...
    {
        p->setPen(QColor(255, 255, 0, 255)); //жёлтый
    }
    if (sigPower < -89 && sigPower >= -94)
    {
        p->setPen(QColor(216, 255, 0, 255)); //жёлтозелёный
    }
    ...
    {
        p->setPen(QColor(0, 255, 143, 255)); //океанский
    }
